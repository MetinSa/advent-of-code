...
fn main() {
    ...
    for line in data.iter() {
        ...
        for c in second.chars() {
            if unique_chars.contains(&c) {
                priority_sum += alphabet.find(c).unwrap() as u32 + 1;
                break;
            }
        }
    }
    println!("Part 1: the sum of the priorities are {}", priority_sum);
    ...
    println!("Part 2: the sum of the priorities are {}", priority_sum);
}
